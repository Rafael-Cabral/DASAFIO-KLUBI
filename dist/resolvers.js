...
const PAGE_MAX = 100;
const maskCpf = (cpf) => cpf.replace(/\d/g, (d, i) => (i < 9 ? '*' : d));
exports.resolvers = {
    Query: {
        async pessoas(_, { q: search, first = 20, after }) {
            if (first > PAGE_MAX)
                throw new Error(`first máximo: ${PAGE_MAX}`);
            const key = `pessoas:q=${search || ''}:first=${first}:after=${after || ''}`;
            const hit = await (0, cache_1.cacheGet)(key);
            if (hit)
                return hit;
            const afterId = (0, pagination_1.decode)(after) ?? 0;
      ...
      `, [search ?? null, afterId, first]);
            const conn = (0, pagination_1.toConnection)(rows, first);
            await (0, cache_1.cacheSet)(key, conn, 60);
            return conn;
        },
        async pessoaPorCpf(_, { cpf }) {
      ...
      `, [norm]);
        ...
        async planos(_, { first = 20, after, min_valor, max_valor, parcelas }) {
            if (first > PAGE_MAX)
                throw new Error(`first máximo: ${PAGE_MAX}`);
      ...
      `, [min_valor ?? null, max_valor ?? null, parcelas ?? null, afterId, first]);
            const conn = (0, pagination_1.toConnection)(rows, first);
            await (0, cache_1.cacheSet)(key, conn, 3600);
            return conn;
        },
        async contratosPorPessoa(_, { pessoaId, status, first = 20, after }) {
            if (first > PAGE_MAX)
                throw new Error(`first máximo: ${PAGE_MAX}`);
      ...
      `, [Number(pessoaId), status ?? null, afterId, first]);
            const conn = {
                edges: rows.map((r) => ({ node: r, cursor: (0, pagination_1.encode)(r.id) })),
                endCursor: rows.length ? (0, pagination_1.encode)(rows[rows.length - 1].id) : null,
        ...
        GROUP BY status
      `);
            await (0, cache_1.cacheSet)(key, rows, 300);
            return rows;
        },
    },
    Mutation: {
        async criarPessoa(_, { nome, cpf, email, telefone }) {
            const r = await (0, db_1.q)(`
      ...
      `, [nome, cpf, email, telefone]);
        ...
        async criarContrato(_, { pessoaId, planoId }) {
            const r = await (0, db_1.q)(`
      ...
      `, [Number(pessoaId), Number(planoId)]);
        ...
        async atualizarStatusContrato(_, { contratoId, status }) {
            const r = await (0, db_1.q)(`
    ...
    Pessoa: {
        cpfMasked: (p) => maskCpf(p.cpf),
    },
    Contrato: {
        pessoa: (c, _, { loaders }) => loaders.pessoasById.load(c.pessoa_id),
        plano: (c, _, { loaders }) => loaders.planosById.load(c.plano_id),
    },
};
